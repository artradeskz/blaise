...
exceed the 32-bit range — notably as the type of the `MaxInt` built-in
constant (value `9223372036854775807`, i.e., 2^63 − 1).
...
`MaxInt: Int64 = 9223372036854775807` is a compiler built-in constant.
It represents the maximum value of a 64-bit signed integer.  It is used
as a sentinel meaning "to end of string" in `Copy()`.
...
| `IntToStr` | `(Integer): string` | 32-bit integer to decimal string
| `Int64ToStr` | `(Int64): string` | 64-bit integer to decimal string; use when the value may exceed 2^31−1
| `StrToInt` | `(string): Integer` | Decimal string to 32-bit integer; truncates silently on overflow
| `StrToInt64` | `(string): Int64` | Decimal string to 64-bit integer; required for parsing large literals such as `MaxInt`
|===
