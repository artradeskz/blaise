...
* Phase 1 targets Linux x86_64 only. macOS ARM64 added in Phase 5 alongside self-hosting. Windows in Phase 5 (LLVM).
...
*Milestone:* Compiler binary produced by itself. QBE backend producing correct
binaries on Linux x86_64 and macOS ARM64; LLVM backend on Windows x86_64.
...
Implementation order: interfaces first (complete), then generics (core
monomorphization complete; `TList<T>` / `TDictionary<K,V>` RTL additions
and standalone generic methods remain pending).
...
| `type TBox<T> = class ... end;` — Delphi syntax (no `generic`/`specialize`);
  single and multiple type parameters; inline method bodies with `Self.field`
  access pattern.  Parsed as `TGenericTypeDef` in the AST.
...
| Demand-driven: on first use of `TBox<Integer>` in a var declaration, the
  ...
  concrete class type in scope, and registers the instance in
  `TProgram.GenericInstances`.  Registry in `TSymbolTable.FGenerics` prevents
  duplicate instantiations.  Symbol is defined in the global scope so it
  survives nested scope pops.
